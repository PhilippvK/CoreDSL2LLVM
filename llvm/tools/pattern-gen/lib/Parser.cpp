...
// static llvm::DenseMap<uint32_t, llvm::SmallVector<Variable>> architectural_state;
static llvm::DenseMap<uint32_t, llvm::SmallVector<llvm::Function>> functions;
...
static void reset_globals2()
{
  // architectural_state.clear();
  functions.clear();
}
...
static bool peek_is_type(TokenStream& ts)
{
  ...
  return peekT == UnsignedKeyword || peekT == SignedKeyword;  // TODO: void?
}
    ...
    case Identifier:
      {
        ...
        // auto iter2 = functions.find(t.ident.idx);
...
void ParseBehaviour (TokenStream& ts, CDSLInstr& instr, llvm::Module* mod, Token const& ident)
{
  ...
  ParseStatement(ts, func, build);
  // llvm::Function *F = getFunction(std::string("binary"));
  // assert(F && "binary operator not found!");
  llvm::SmallVector<llvm::Type*, 8> argTypes2;
  llvm::SmallVector<llvm::StringRef, 8> argNames2;
  llvm::Value* L = build.getInt32(0);
  llvm::Value* R = build.getInt32(42);
  argTypes2.push_back(L->getType());
  argTypes2.push_back(R->getType());
  // argNames2.push_back("foo");
  // argNames2.push_back("bar");
  ...
  //   ...
  // };
  auto fType2 = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx), argTypes2, false);
  llvm::Function* riscv_brcc =
    llvm::Function::Create(fType2, llvm::GlobalValue::ExternalLinkage,
        std::string("riscv_brcc"), mod);
  // for (size_t i = 0; i < argNames2.size(); i++)
  //     riscv_brcc->getArg(i)->setName(argNames2[i]);

  llvm::Value *Ops[2] = { L, R };
  // build.CreateCall(riscv_brcc, Ops, "binop");
  build.CreateCall(riscv_brcc, Ops);
  build.CreateRetVoid();
}
  ...
  while(ts.Peek().type != None)
  {
    ...
    if (parseBoilerplate)
    {
      ...
      if (pop_cur_if(ts, ExtendsKeyword))
        pop_cur(ts, Identifier);
      pop_cur(ts, CBrOpen);
      if (pop_cur_if(ts, FunctionsKeyword)) {
        pop_cur(ts, CBrOpen);
        while (ts.Peek().type != CBrClose && ts.Peek().type != None)
        {
          bool isExtern = ts.Peek().type == ExternKeyword;
          llvm::outs() << "isExtern=" << isExtern << "\n";
          if (isExtern) {
            llvm::outs() << "A" << "\n";
            pop_cur(ts, ExternKeyword);
            llvm::outs() << "B" << "\n";
            llvm::Type* retType = llvm::Type::getVoidTy(ctx);
            switch (ts.Pop().type) {
              case SignedKeyword:
              case UnsignedKeyword: {
                                      llvm::outs() << "Unsigned/Signed" << "\n";
                                      int len = 0;
                                      if (pop_cur_if(ts, LessThan))
                                      {
                                        len = pop_cur(ts, IntLiteral).literal.value;
                                      ...
                                      if (len != 0)
                                      {
                                        retType = llvm::Type::getIntNTy(ctx, len);
                                      }
                                      break;
                                    }
              case VoidKeyword: {
            ...
            llvm::SmallVector<llvm::Type*, 8> argTypes;
            llvm::SmallVector<llvm::StringRef, 8> argNames;
            while (ts.Peek().type != RBrClose && ts.Peek().type != None) {
              llvm::outs() << "F" << "\n";
              llvm::Type* argT = llvm::Type::getVoidTy(ctx);
              switch (ts.Pop().type) {
                case SignedKeyword:
                case UnsignedKeyword: {
                                        llvm::outs() << "Unsigned/Signed" << "\n";
                                    ...
                                    if (len != 0)
                                    {
                                        argT = llvm::Type::getIntNTy(ctx, len);
                                    }
                                    llvm::outs() << "F4" << "\n";
                                    break;
                                }
                            ...
                            llvm::outs() << "G" << "\n";
                            argTypes.push_back(argT);
                            Token ident2 = pop_cur(ts, Identifier);
                            argNames.push_back(ident2.ident.str);
                            llvm::outs() << "H" << "\n";
                        }
                        llvm::outs() << "I" << "\n";
                        pop_cur(ts, RBrClose);
                        ...
                        llvm::Function* riscv_brcc =
                            llvm::Function::Create(fType, llvm::GlobalValue::ExternalLinkage,
                                                    std::string(ident.ident.str), mod);
                        pop_cur(ts, Semicolon);
                        llvm::outs() << "K" << "\n";

                    } else {
                        switch (ts.Peek().type) {
                            case UnsignedKeyword: break;
                            case SignedKeyword: break;
                            case VoidKeyword: break;
                            default: break;
                        }
                        Token ident = pop_cur(ts, Identifier);
                        pop_cur(ts, CBrOpen);
