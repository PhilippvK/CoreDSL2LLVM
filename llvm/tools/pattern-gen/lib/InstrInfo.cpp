...
std::string EncodingToTablgen(CDSLInstr const &instr) {
  ...
  std::string base =
      (size == 48 ? "RVInst48" : (size == 16 ? "RVInst16" : "RVInst"));
  ...
  s << "class RVInst_" << InstNameO << "<dag outs, dag ins>"
    << " : " << base << "<outs, ins, \"" << instr.mnemonic << "\", \""
    << instr.argString << "\", [], InstFormatOther> {\n";
  ...
  for (auto const &f : instr.frags) {
    s << "\tlet Inst{" << std::to_string(f.dstOffset + f.len - 1) << "-"
      << std::to_string(f.dstOffset) << "} = ";
    auto const &field = instr.fields[f.idx];

    if (field.type == CDSLInstr::FieldType::CONST)
      s << "0x" << std::hex
        << ((field.constV >> f.srcOffset) & ((1UL << f.len) - 1)) << std::dec;
    else
