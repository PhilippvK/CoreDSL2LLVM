...
define void @implLDR(i64 %imm, ptr readonly captures(none) %rs2, ptr readonly captures(none) %rs1, ptr noalias writeonly captures(none) initializes((0, 8)) %rd) local_unnamed_addr #0 {
...
define void @implSTR(ptr readonly captures(none) %rs3, ptr readonly captures(none) %rs2, ptr readonly captures(none) %rs1, i64 %imm) local_unnamed_addr #2 {
  %1 = icmp ult i64 %imm, 32
  tail call void @llvm.assume(i1 %1)
  %rs2.v = load i64, ptr %rs2, align 8
  %2 = shl i64 %rs2.v, %imm
...
define void @implLOADMAC(ptr readonly captures(none) %rs2, ptr readonly captures(none) %rs1, ptr noalias captures(none) %rd) local_unnamed_addr #3 {
  %rs1.v = load i64, ptr %rs1, align 8
